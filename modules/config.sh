...
setup_ssh() {
    test -n "${SSH_KEY:-}" && test -n "${SSH_PUB:-}" || return 0
    log "Installing SSH keypair (~/.ssh/id_rsa[.pub])"
    mkdir -p ~/.ssh
    chmod 700 ~/.ssh
...
setup_git() {
    ...
    test -z "${GIT_NAME:-}" || gitconf user.name "$GIT_NAME"
    test -z "${GIT_MAIL:-}" || gitconf user.email "$GIT_MAIL"
...
# infix - sed'ed OMZ .zshrc
sed_zshrc() { sed -E "s|^(# )?$1=.*\$|$1=$2|"; }
...
[ -s "$FZF_DIR/shell/completion.zsh" ] && source "$FZF_DIR/shell/completion.zsh"
[ -s "$FZF_DIR/shell/key-bindings.zsh" ] && source "$FZF_DIR/shell/key-bindings.zsh"
...
profile() {
sed -E "s|\{\{PATH\}\}|$BIN:$VENV/bin:$GO/bin|" <<'EOF'
...
p10k() {
cat <<'EOF'
# Generated by Powerlevel10k configuration wizard on 2022-01-05 at 18:21 CET.
